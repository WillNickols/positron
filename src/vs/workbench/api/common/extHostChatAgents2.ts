	...
	private async getModelForRequest(request: IChatAgentRequest, extension: IExtensionDescription): Promise<vscode.LanguageModelChat> {
		...
		if (!model) {
			...
			if (!model) {
				...
				throw new Error('No language models available for chat. Please ensure you have logged into a language model provider by clicking `Add Model Provider...` and authenticating with a provider like Anthropic.');
				/*
